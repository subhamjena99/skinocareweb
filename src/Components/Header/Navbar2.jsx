...
const Navbar2 = ({ openSidebar, userDetails, siteDetails }) => {
    ...
    useEffect(() => {
        getRequest({ url: 'api/user/product/recent/search', token: authToken })
            .then(res => {
                if (res?.data?.status) {
                    setRecentSearchList(res?.data?.data);
                };
            })
            .catch(err => {
                // if(err?.data?.msg){
    ...
    // RECENTLY SEARCHED PRODUCT API CALL
    const searched = (slug) => {
        if (authToken !== 'undefined' && authToken !== undefined) {
            postRequest({ url: 'api/user/product/recent/store', cred: { slug }, token: authToken })
                .then(res => {
                    if (res?.data?.status) {
                ...
                <div className="navbarboxs">
                    ...
                    <div className="navbarbox">
                        <div className="navsearchbox">
                            <form onSubmit={searchProdutsList}>
                                <input type="text" onChange={handleSearch} value={searchText} className='search-input' placeholder='Search for Product' />
                                <svg xmlns="http://www.w3.org/2000/svg" className='ms-3 a' width="18" height="19" viewBox="0 0 18 19" fill="none">
                        ...
                        {(authToken !== undefined && authToken !== "undefined") ?
                            <div className='search-content'>
                                ...
                                <div className="search-result">
                                    {
                                        searchText ?
                                            suggestionSearchList?.map((elem, index) => <Link href={elem?.slug ? '/shop/' + elem?.slug : '#!'} key={index + 1} onClick={() => searched(elem?.slug)}><p>{elem?.name}</p></Link>)
                                            :
                                            recentSearchList?.map((elem, index) => <Link href={elem?.slug ? '/shop/' + elem?.slug : '#!'} key={index + 1} onClick={() => searched(elem?.slug)}><p>{elem?.searchTxt}</p></Link>)
                            ...
                            :
                            <>
                                {suggestionSearchList?.length ? <div className='search-content'>
                                    <div className="search-action">
                                        <p className='m-0'>SUGGESTIONS</p>
                                    </div>
                                    <div className="search-result">
                                        {
                                            suggestionSearchList?.map((elem, index) => <Link href={elem?.slug ? '/shop/' + elem?.slug : '#!'} key={index + 1} onClick={() => {
                                                setSuggestionSearchList([]);
                                                setSearchText('');
                                            }}><p>{elem?.name}</p></Link>)
                                        }
                    ...
                    <div className="navbarbox">
                        <div className="navbarbutton">
                            ...
                            <div className="cardbutton1">
                                <span className="navlinbox a" onClick={() => setWishlistToggler(true)} title='Wishlist'>
                            ...
                            {!token && <div className="navbarbutton ">
                                <Link href='/signin'>Sign in</Link>
                            </div>}


                            {token && <div className="profilesec ms-2">
                                    ...
                                    <div className="accordion">
                                        ...
                                        <div id="collapseTwo123" className="accordion-collapse collapse" data-bs-parent="#accordionExample">
                                            <div className="accordion-body profilebox1">
                                                <p><Link href="/account" className='bg-transparent'><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                                    <path d="M2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12Z" stroke="#101828" strokeWidth="1.5" strokeLinecap="round" />
                                                    <path d="M8 10C8 12.2091 9.79086 14 12 14C14.2091 14 16 12.2091 16 10C16 7.79086 14.2091 6 12 6C9.79086 6 8 7.79086 8 10Z" stroke="#101828" strokeWidth="1.5" strokeLinecap="round" />
                                                    <path d="M6.34961 19.5C7.6538 17.9502 9.60088 17 11.7195 17H12.2792C14.3978 17 16.3449 17.9502 17.6491 19.5" stroke="#101828" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                                </svg> Account</Link></p>
                                                <Link href="#!" className='bg-transparent' onClick={() => logout()}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
